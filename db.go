...
func Open(options option.Options) (*DB, error) {
	// 对用户传入的配置项进行校验
	if err := option.CheckOptions(options); err != nil {
		return nil, err
	}
	// 判断数据目录是否存在，不存在就得创建
...
func (db *DB) Get(key []byte) ([]byte, error) {
	...
	if ok, _ := db.bloomFilter.Contains(key); !ok {
